...
for (const flight of flights.split('+')) {
  ...
  const output = `${type.replaceAll('_', ' ').slice(1)} from ${from
    .slice(0, 3)
...
const restaurant = {
  ...
  orderDelivery({ starterIndex = 1, mainIdex = 0, time = '20:00', address }) {
    console.log(
      `Order received! ${this.starterMenu[starterIndex]} and ${this.mainMenu[mainIdex]} will be delivered to ${address} at ${time}`
    );
  },
...
document.querySelector('button').addEventListener('click', function () {
  ...
  for (const [i, row] of rows.entries()) {
    ...
    console.log(camelCase.trim().padEnd(20) + `${'✅'.repeat(i + 1)}`);
  }
});
...
capitalizeName('leonardo fernandes');

// Padding
const message = 'Go to gate 23!';
console.log(message.padStart(20, '+').padEnd(30, '+'));
console.log('Leonardo'.padStart(20, '+').padEnd(30, '+'));
...
console.log(maskCreditCard(15975385));
console.log(maskCreditCard(98754321598763245));
console.log(maskCreditCard('564987563215475678546'));
